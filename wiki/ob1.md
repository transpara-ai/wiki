...
sources:
  ...
  - raw/transpara/dark-factory/v4.0/04-production-workflow-runtime-v4.0.md  # accepted v4.0: Release-only certification; external runtimes are references/adapters only; Base Slice 0 (cited by rule — doc 04 is under active revision)
...
confidence:
  ...
  fork_divergence: "resolved-negative — the fork carries NO Transpara feature or catalog divergence; its delta is entirely deployment/server-safety (PRs #1–#6 — local-Docker + k8s self-host, fingerprint-dedup, getEmbedding guards, verifiable/fail-closed capture — of which #5/#6 are the most substantive — PR #6 the only commit after the 2026-06-14 page, PR #5 already summarized in its Run-3 note). (Supersedes the prior page's unverified \"agentic harnesses skill pack\" note, which is upstream community content, not fork-added.)"
...
>
> **Material links —**
> **Adopted** (the *pattern* + running the forked code internally as advisory tooling; FSL-permitted, never a control role): [[event-graph]] transaction-log-vs-reasoning-log split (2026-04-09) · the internal Open Brain store (`njofekbuaauffxqsfikl`) · accepted v4.0's memory/knowledge-advisory rule (doc `03`: *"memory and knowledge are advisory only"*).
...
- **PR #5 (`1d1d6e2`, 2026-06-14)** made `capture_thought` verifiable and embedding-failure-safe — a fail-closed contract (*committed-but-no-id ⇒ "treat as NOT saved"*), moving the durability guarantee server-side. **PR #6 (`1c4c906`, 2026-06-17)** cleaned `deno check` resolution and guarded an empty embedding-error message. PR #5/#6 are the most substantive of the fork's Transpara commits; PR #6 is the only one after the 2026-06-14 page (PR #5 was already summarized in that page's Run-3 note). The rest (PRs #1–#4, plus a self-host recipe and key-rotation docs) are deployment/server-safety only — **no feature or catalog divergence**.
