...
confidence:
  ...
  fork_divergence: resolved-negative — the fork carries NO Transpara feature divergence; its only substantive delta is four server/deployment safety files across PR #5/#6. (Supersedes the prior page's unverified "agentic harnesses skill pack" note, which is upstream community content, not fork-added.)
...
| Fork divergence | "not established… maybe an agentic-harnesses skill pack (PR #151)" | **Resolved: no Transpara feature divergence.** The skill pack is *upstream* community content; the fork's only delta is four server/deployment safety files (PR #5/#6). `git ls-files \| grep transpara` = 0 |
...
- **Fork divergence — now resolved (negative).** The prior page's open question ("does the fork carry Transpara-specific commits?") is answered: **no feature divergence.** The fork's only substantive delta is the four server/deployment safety files of PR #5/#6; the "agentic harnesses skill pack (PR #151)" a 2026-04 thought attributed to the fork is **upstream community content** (`skills/n-agentic-harnesses`, by an upstream contributor), and PR #151 is an upstream number (the fork's PRs run #1–#6) — the code read supersedes that thought.
