...
sources:
  ...
  - raw/transpara/dark-factory/v4.0/03-civilization-governance-and-authority-v4.0.md  # accepted v4.0: EventGraph is truth, "memory and knowledge are advisory only" (03:36)
  - raw/transpara/dark-factory/v4.0/04-production-workflow-runtime-v4.0.md  # accepted v4.0: source_refs (04:48); Release-only certification (04:110); Base Slice 0 + external-adapter eligibility gate (04:135, 04:165)
  - raw/transpara/dark-factory/v4.0/05-verification-audit-risk-eval-v4.0.md  # accepted v4.0: certification-blocking evidence set (05:36); memory/knowledge-vs-EventGraph kill criterion (05:523)
...
|---|---|---|
| Index construction (PDF) | Upstream claim: "builds a ToC tree, an LLM traverses it" | For PDFs, every node/title/hierarchy-number/page-map/summary is an LLM output; **no deterministic parser in that path** (`page_index.py:1058`, `:531`). The structure is inferred, not parsed. The secondary Markdown path parses `#` headers deterministically (`page_index_md.py:32`, `:190`). |
...
| Runtime dependency | Not examined | **Vectorless but not API-less** — building a **PDF** index requires an external LLM (litellm, default `gpt-4o`; `utils.py:33`, `config.yaml:1`); Markdown needs one only for optional summaries. Inverse of MemPalace's local-first posture; collides with the air-gap default and Base Slice 0's "no knowledge reuse." |
...
For a **PDF** — PageIndex's default input — the tree is built entirely by LLM calls, producing `{title, node_id, start_index, end_index, summary, nodes[]}` records. The entry point rejects any non-PDF input (`page_index.py:1065`), extracts page text with `PyPDF2` (`utils.py:388`), and dispatches (`:1025`): a table of contents **with printed page numbers** runs `process_toc_with_page_numbers` (`:611`); **anything else** — no TOC, or a TOC without page numbers — runs `process_no_toc` (`:565`), where the LLM generates the hierarchy and the "1 / 1.1 / 1.2" numbering from scratch (`:531`, `:503`). (`process_toc_no_page_numbers` (`:586`) is not a primary path — only a fallback rung when a numbered-TOC build fails its self-check.) Large sections are recursively subdivided (`:992`); node summaries and the doc description are additional LLM passes (`utils.py:590`, `:623`). Calls run at `temperature=0` (`utils.py:43`), which lowers variance but is not deterministic across models.

The **Markdown path is the exception**: `md_to_tree` (`page_index_md.py:243`) parses `#` headings with a regex and builds the tree from heading levels deterministically (`:32`, `:190`) — no LLM for the structure, only for optional summaries (off by default in `md_to_tree`, on via `PageIndexClient`). The README discourages Markdown for PDF-derived content, so the LLM-built PDF path carries every benchmark and marketing claim.
