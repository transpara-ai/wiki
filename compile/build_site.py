...
SEARCH_JS = (
    ...
    'function score(row,terms,q,partial){if(!inScope(row))return -1;var title=norm(row.title),tier=norm((row.section||row.tier||"")+" "+(row.space_labels||[]).join(" ")),text=norm(row.text);var s=0,matched=0;'
    'for(var i=0;i<terms.length;i++){var t=terms[i];'
    'if(title.indexOf(t)!==-1)s+=25;else if(tier.indexOf(t)!==-1)s+=8;'
    'else if(text.indexOf(t)!==-1)s+=3;else {if(!partial)return -1;continue;}matched++;}'
    'if(!matched)return -1;if(title.indexOf(q)===0)s+=30;return s+100*matched/terms.length;}'
    ...
    'if(!terms.length)terms=Array.from(new Set(words));'
    'function ranked(partial){return docs.map(function(row){return {row:row,score:score(row,terms,q,partial)};})'
    ...
    'hits=ranked(false);if(!hits.length)hits=ranked(true);'
...
def search_box(prefix="", active_space=""):
    ...
    return (
        ...
        '<label class="ask-hint" for="wiki-ask-effort">Effort</label>'
        '<select id="wiki-ask-effort" aria-label="Effort"><option value="">Loading…</option></select>'
        '<span id="wiki-ask-hint" class="ask-hint">Press Enter to ask</span></div>'
...
def github_link():
    # GitHub's standard Octicon; license: assets/octicons-LICENSE.txt.
    return ('<a class="github-link" href="https://github.com/transpara-ai/wiki" '
            'target="_blank" rel="noopener noreferrer" '
            'aria-label="View source on GitHub (opens in a new tab)" title="View source on GitHub">'
...
def header_controls(status, active_space=""):
    return (space_freshness(status, active_space) +
            ...
            'aria-label="Toggle dark or light theme">☾ dark</button>' + github_link() + account_menu())
...
def account_menu():
    ...
    return (
        ...
        '<h3>Rights</h3><p id="account-authoring">Wiki authoring access has not been checked.</p><p>Other detailed rights are not available here yet.</p></div>'
        ...
        '<button type="button" id="account-forget-authoring" hidden>Forget authoring access</button>'
...
def ingest_page(status):
    ...
    inner = (
        ...
        '<label id="authoring-token-label">Authoring token<input id="authoring-token" type="password" autocomplete="off" placeholder="required for non-loopback authoring"></label>'
        '<p id="authoring-profile-status" role="status"></p>'
        '<button id="remember-authoring" type="button" hidden>Remember authoring access</button>'
        '</section>'
        ...
        'function headers(){var h={"X-Wiki-Profile-Action":"1"};if(token&&token.value)h["X-CivWiki-Authoring-Token"]=token.value;return h;}'
        'function readResponse(r){return r.text().then(function(body){var j;'
        'try{j=JSON.parse(body);}catch(e){'
        'var message=(r.status===502||r.status===504)?'
        '"The proxy stopped waiting for the wiki (HTTP "+r.status+"). The operation may still finish. Check the updated time before retrying.":'
        '"The server returned an unexpected response (HTTP "+r.status+"). Reload this page and sign in again if prompted.";'
        'throw {error:message};}'
        ...
        '.then(readResponse).then(reloadWithResult).catch(function(e){say(e);});});'
