...
RUN apt-get update \
    && apt-get install -y --no-install-recommends git rsync util-linux openssh-client \
    && rm -rf /var/lib/apt/lists/*
RUN groupadd -g 1000 wiki && useradd -u 1000 -g wiki -M -d /nonexistent -s /usr/sbin/nologin wiki
