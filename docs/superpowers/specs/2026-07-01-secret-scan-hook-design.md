...
- **Oversized blobs** (> 1 MiB) **block**: "too large to scan — review manually and attest with a blob_review entry". The decision is made **from `cat-file -s` alone — the content is never materialized** (a multi-GB accident blocks cheaply instead of OOMing the hook/runner; CFAR F12); content is only ever **streamed** through a chunked hash to verify an existing attestation.
...
- **Snapshot rule (CFAR F2/F9/F13)**: the allowlist is read from a **snapshot, never the working tree** — `--staged` reads the **index** copy (the review record must travel with the commit it clears), `--tree <rev>` reads `<rev>`'s copy, and `--changed-against` splits the two roles: **clearance** comes only from the **base ref's valid entries** (**merged = human-ratified; a PR cannot add a secret and self-allowlist it in the same change**; CFAR F9, P1), while the **head copy is strict-validated but never used for clearance** — an expired/malformed entry inherited in the PR's own allowlist blocks (expiry keeps forcing renewal repo-wide), yet an invalid entry in the *base* copy is **VOID with a loud report instead of fatal**, so the renewal PR — whose head copy is fixed — always has a green path (**no expiry wedge**; CFAR F13). Absent from the snapshot = empty allowlist; a git error while reading ⇒ block. Workflow consequence, stated plainly: a *new* false positive clears CI only after its allowlist entry has **merged**; after an expiry, nothing merges until a renewal PR does — and that PR can. Named tests `test_allowlist_snapshot_matches_scan_mode`, `test_ci_does_not_trust_pr_head_allowlist`, `test_expired_base_entry_does_not_wedge_renewal`.
...
| AC | Named test(s) |
|---|---|
| AC1 | `test_planted_secret_blocked` · `test_secret_in_pathname_blocked` (CFAR F14: pathnames are scanned; path findings non-clearable, remediation = rename) |
...
| AC5 | `test_stdlib_only_no_network` (import allowlist over `secret_scan.py`: stdlib modules only, no `socket`/`urllib`/`http` usage) · `test_partial_clone_missing_blob_blocks_without_fetch` (CFAR F8: every git subprocess runs with `GIT_NO_LAZY_FETCH=1` — a missing promisor blob blocks instead of lazy-fetching over the network) |
| AC6 | `test_allowlist_entry_does_not_cover_sibling_findings` · `test_duplicate_identical_matches_need_separate_entries` (CFADA-r5 B1) · `test_allowlist_entry_does_not_cover_other_paths` · `test_blob_review_rejected_for_text_blobs` · `test_allowlist_fingerprint_reflags_on_edit` · `test_allowlist_snapshot_matches_scan_mode` (CFAR F2) · `test_ci_does_not_trust_pr_head_allowlist` (CFAR F9) · `test_scanner_tree_scans_clean` |
...
| AC10 | `test_allowlist_schema_and_expiry_enforced` (incl. future-dated `reviewed_on` CFAR F1, duplicate JSON keys CFAR F10, exact date shape CFAR F11) · `test_allowlist_dates_compared_in_utc` (CFAR F6) · `test_expired_base_entry_does_not_wedge_renewal` (CFAR F13) |
