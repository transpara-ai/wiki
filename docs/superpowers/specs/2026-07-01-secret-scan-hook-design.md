...
- **Snapshot rule (CFAR F2/F9)**: the allowlist is read from a **snapshot, never the working tree** — `--staged` reads the **index** copy (the review record must travel with the commit it clears), `--changed-against` reads the **base ref's** copy (**only merged, human-ratified entries can clear CI findings — a PR cannot add a secret and self-allowlist it in the same change**; CFAR F9, P1), `--tree <rev>` reads `<rev>`'s copy. Absent from the snapshot = empty allowlist; a git error while reading ⇒ block. Workflow consequence, stated plainly: a *new* false positive clears CI only after its allowlist entry has **merged** — land the entry (inert until its pinned blob exists), then the content PR passes; or rebase the content PR once the entry is on the base. Named tests `test_allowlist_snapshot_matches_scan_mode`, `test_ci_does_not_trust_pr_head_allowlist`.
