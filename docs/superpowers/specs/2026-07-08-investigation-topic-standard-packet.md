...
doc_type: design
version: 0.10.2
status: "AMENDED v0.10.2 (2026-07-10, Phase-2 PR #74 CFAR r2 repairs — quote this scalar as valid YAML; align AC1's fallback wording + named-test list with the third key). v0.10.1 formalizes `superseded_sources` as the THIRD Topic-Details key (§2.2/AC1/AC6-P2) — the legacy-page Replace lane moves a superseded ingested ref that lived only under `sources` into `superseded_sources`, and Topic Details unions it so R3's 'every raw ingested version' holds on that path too; a record-of-reality amendment documenting the mechanism the Phase-1 implementation added under CFAR (Codex finding; shipped in PR #73, `topic_details_refs`, guarded by `test_topic_details_includes_superseded_sources`), no new machinery. Prior — v0.10.0 CFADA re-PASS (2026-07-09), folds in R9 (single nav entry via `investigation_primary`) + revises R8 per Michael's Option-A nav decision; IADA + CFADA re-run (7 amendment rounds 17–23, findings C35–C46 all repaired) returned 0 design blockers at the round-23-audited blob f54544fa444774a2ea12e03843d426563afd9ce1; HDR granted via merged PR #70."
canonical: false
created: 2026-07-09
updated: 2026-07-10
...
sourced from `raw_documents` **∪** `superseded_raw_documents` **∪**
`superseded_sources` (deduped, newest primary, superseded entries marked),
because the Replace op moves superseded refs into `superseded_raw_documents`
(`ingest_ops.py:952`) and R3 requires all versions, superseded included
(CFADA-r3 #8). **`superseded_sources` is the third key (v0.10.1):** a LEGACY
page whose ingested doc lived only under `sources` (never in `raw_documents`)
has nothing for a Replace to move between the raw-doc keys, so its Replace
lane records the superseded ref in `superseded_sources`; Topic Details unions
that key too, and `topic_details_superseded` marks its entries, so no raw
ingested version is lost on the legacy path either (Phase-1 CFAR finding,
Codex; guarded by `test_topic_details_includes_superseded_sources`). For an ADD-with-supersedes topic that keeps
...
|---|---|---|---|
| AC1 | 1 | Topic Details row lists every `raw_documents`, `superseded_raw_documents`, AND `superseded_sources` entry (superseded included, marked — the third key covers the legacy sources-only Replace lane, v0.10.1; `test_topic_details_includes_superseded_sources`), falling back to raw-INGESTED RESEARCH refs (`raw/inbox/...` uploads ∪ `…/tai-res-*.md` evaluations wherever placed, e.g. pageindex's `raw/civilization/external-landscape/…` — CFADA-r17 #35) only when all THREE keys are empty — doctrine/provenance/absolute citations render in the source panel, NOT Topic Details (CFADA-r14 #31); no "Raw docs" label survives in `dist/`; for an ADD-with-supersedes topic (MemPalace) the older versions render marked, the newest primary, from the `supersedes:` comments (CFADA-r21 #43) (low) | py: `test_topic_details_lists_all_versions_incl_superseded`, `test_topic_details_marks_add_supersedes_newest_primary`, `test_topic_details_fallback_is_raw_ingested_only`, `test_topic_details_includes_superseded_sources` |
