...
doc_type: design
version: 0.9.0
status: draft (IADA-passed; CFADA r1-r8 repaired; re-audit pending)
canonical: false
created: 2026-07-09
updated: 2026-07-09
...
> IADA (v0.2.0) and CFADA rounds 1 (v0.3.0), 2 (v0.4.0), 3 (v0.5.0), 4 (v0.6.0), 5 (v0.7.0), 6 (v0.8.0), 7 (v0.8.1, audit-trail), 8 (v0.9.0).
...
| 1 | EDIT | `compile/ingest_server.py` | **R5/R6/R1 (§2.4):** intent model + fail-closed guard; `create_article_from_source(name=…)` keyed on the operator name (CFADA-r2 #4); `prospective_unassigned_slug` in lockstep; collision set includes `investigation_topic` cluster labels (CFADA-r3 #9), matched on `collision_key` forms incl. parentheses (CFADA-r5 #13, r8 #21); the operator name is quarantined pre-write (CFADA-r6 #16); ADD keeps `require_authoring` (CFADA-r6 #15). Default lane appends (any tier), stamps `stale_since` only for investigation targets. |
...
Unicode-casefold `s`, then replace every run of non-alphanumeric characters
(whitespace, hyphens, AND punctuation INCLUDING parentheses) with a single
hyphen, then strip leading/trailing hyphens. Unlike `ingest_server.slugify`,
...
page's slug, active OR tombstone}. So "Sakana AI", "Sakana-AI", and
"Sakana (AI)" all key to `sakana-ai` and collide (CFADA-r3 #9, r5 #13, r8 #21).
The created page's slug still comes from `slugify(name)`; only the collision
test uses `collision_key`. The
...
Re-audit (CFADA round 9) pending at v0.9.0. No code before Human Design Review
(stage 6) approves.
