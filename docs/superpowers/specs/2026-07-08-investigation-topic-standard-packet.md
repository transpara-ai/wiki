...
doc_type: design
version: 0.10.1
status: AMENDED v0.10.1 (2026-07-10, Phase-2 PR) — formalizes `superseded_sources` as the THIRD Topic-Details key (§2.2/AC1/AC6-P2): the legacy-page Replace lane moves a superseded ingested ref that lived only under `sources` into `superseded_sources`, and Topic Details unions it so R3's "every raw ingested version" holds on that path too. This documents the mechanism the Phase-1 implementation added under CFAR (Codex finding; shipped in PR #73, `topic_details_refs`, guarded by `test_topic_details_includes_superseded_sources`) — a record-of-reality amendment, no new machinery. Prior: v0.10.0 CFADA re-PASS (2026-07-09) — folds in R9 (single nav entry via `investigation_primary`) + revises R8 per Michael's Option-A nav decision; IADA + CFADA re-run (7 amendment rounds 17–23, findings C35–C46 all repaired) returned 0 design blockers at the round-23-audited blob f54544fa444774a2ea12e03843d426563afd9ce1; HDR granted via merged PR #70.
canonical: false
created: 2026-07-09
updated: 2026-07-10
