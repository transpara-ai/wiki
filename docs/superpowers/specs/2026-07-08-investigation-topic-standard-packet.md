...
- **Empty render-driving fields (CFADA-r19 #40):** a required key present but
  blank/empty is a deficiency, not only an ABSENT key — presence ≠ non-empty. In
  particular a blank `civilization_contribution:` renders NO contribution box
  though the key "exists"; every required frontmatter field must be present AND
  non-empty.
...
Both accepted-repaired (0 rejected, 0 waived); round 20 re-audits the new blob SHA. **Amendment CFADA verdict: PENDING round-20 re-review** (this section is the round record, not a PASS).
