...
version: 0.6.2
status: amended post-CFADA-r6 — the CONFIRMED READING remains the v0.1.0 object (blob 7c10b2ffe4f9e6903328669baf68ab2ff004df86, confirmed verbatim 2026-07-13); Michael's explicit byte-confirmation of THIS v0.6.2 file is the NAMED PRECONDITION for Human Design Review entry (supersedes every prior pending status)
...
> lanes (CFADA-r5). **v0.6.2** is a reference-consistency repair (CFADA-r6):
> every self-reference now names this file's version and blob lineage; no
> requirement changed.
...
- **R2 — Organized by ingestion date, newest first; total deterministic
  ...
  original name. Date source: valid file-shape manifest rows' `ingested_at`;
...
- **R5 — Reachable from the site navigation.** One nav entry; the permitted
  ...
  navbox unchanged. Verifiable: the nav test asserts exactly the named delta.
...
Unchanged from v0.5.0: no ingest-op/authorization/scanner changes (the
reader's stricter row requirements are design-local — strengthening
`ingest_ops` itself would be a separately authorized order); no `raw/**`
...
- Version chain: v0.2.0 confirmation record (blob `8c3c16e7…`) → v0.3.0
  ...
  → v0.6.1 CFADA-r5 (blob `c8c51d64…`) → **v0.6.2 CFADA-r6 (this file —
  reference-consistency repair)**.
- **Michael's explicit confirmation of the v0.6.2 bytes is a named
  precondition for Human Design Review entry.** Fail closed.

## 5. v0.1.0 → v0.6.2 delta record (what the byte-confirmation covers)
...
This FO grants nothing and authorizes no code. Next: packet v0.6.2 → IADA r7
→ CFADA r7 → Michael's v0.6.2 byte-confirmation + Human Design Review.
