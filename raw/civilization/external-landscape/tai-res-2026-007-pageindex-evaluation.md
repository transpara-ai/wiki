...
The central finding is that **the 2026-05-13 investigation's disposition was correct, and the code now proves the load-bearing half of it.** PageIndex is a genuinely useful document-structuring and evidence-locating primitive, and it is the reference implementation of the Civilization's already-named optional `DocumentEvidenceRetriever` pattern. But for a **PDF** — PageIndex's default input, the one the demo and the 98.7% benchmark use, and the one behind the "AlphaGo tree search" framing — its "structure" is not a parsed artifact, it is an **LLM-generated one**. Every node, title, hierarchy number, page mapping, and summary in a PDF tree is produced by an `llm_completion` call (default `gpt-4o-2024-11-20`), and the only "validation" is a second LLM being asked whether a sampled title appears on its mapped page, accepted at a hard-coded 0.6 self-agreement threshold with a fallback cascade (`pageindex/page_index.py:892`, `:951`, `:978`). Building a PDF index therefore **requires an external LLM API** (`pageindex/utils.py:33`, `:22`; `config.yaml:1`) — the inverse of MemPalace's local-first posture, and a direct collision with both the air-gap default and Base Slice 0's "no LLM planning, no knowledge reuse" rule (`DF-V4.0-RUNTIME-004`, doc `04:135`). (PageIndex's secondary **Markdown** path is the deterministic exception — it parses the tree from `#` headers and calls no LLM unless optional summaries are requested — but the upstream steers real documents to the PDF path.) This is the code-level proof of the investigation's #1 recorded risk, **"retrieval mistaken for truth"** [11].
...
PageIndex's default and primary input is a **PDF**; a secondary **Markdown** path exists and behaves very differently (below). For a PDF, the output is a hierarchical tree of `{title, node_id, start_index, end_index, summary, nodes[]}` records. The public entry `page_index()` (`pageindex/page_index.py:1105`) routes into `page_index_main()` (`:1058`), which **rejects any non-PDF input** — `raise ValueError("Unsupported input type. Expected a PDF file path or BytesIO object.")` (`:1065`) — extracts page text with `PyPDF2` (or optionally PyMuPDF) via `get_page_tokens()` (`pageindex/utils.py:388`), and hands the page list to `tree_parser()` (`:1021`).

The decisive fact is that **for a PDF the tree is produced entirely by LLM calls; there is no deterministic parser in this path.** `tree_parser()` calls `check_toc()` (`:685`, itself an LLM per-page yes/no via `toc_detector_single_page` `:104`) and then dispatches on the result (`:1025`): if a table of contents **with printed page numbers** was found it runs `meta_processor()` in mode `process_toc_with_page_numbers` (`:611`); **otherwise** — no TOC at all, *or* a TOC without usable page numbers — it runs `process_no_toc` (`:565`), where the LLM *generates* the hierarchy and the "1 / 1.1 / 1.2" structure numbering from scratch, page group by page group (`generate_toc_init` `:531`, `generate_toc_continue` `:496`, `:503`). A third mode, `process_toc_no_page_numbers` (`:586`), is **not** a primary dispatch target — it is reached only as a *fallback* rung inside `meta_processor` after a numbered-TOC build fails its self-check (§2.3). In the numbered-TOC mode the LLM transforms the raw TOC into JSON (`toc_transformer` `:264`), maps entries to physical page tags (`toc_index_extractor` `:234`), and a page offset is computed as the statistical **mode of the printed-vs-physical differences** (`calculate_page_offset` `:383`).

Large sections are then subdivided **recursively** — `process_large_node_recursively()` (`:992`) re-runs the generator on any node exceeding both `max_page_num_each_node` (10) and `max_token_num_each_node` (20,000) (`config.yaml:5`). Optional post-passes add node IDs (`write_node_id`, `utils.py:133`), per-node **LLM summaries** (`generate_summaries_for_structure`, `utils.py:590`), and a one-line **LLM document description** (`generate_doc_description`, `utils.py:623`). Every field in a PDF tree originated from a model, not from the document's own machine-readable structure; the calls run at `temperature=0` (`utils.py:43`), which reduces run-to-run variance but does not make extraction deterministic across model versions or providers.

The **Markdown path is the deterministic exception.** `md_to_tree()` (`pageindex/page_index_md.py:243`) extracts headings with a regex (`^#{1,6}\s+…`, `:32`), takes each heading's level from the number of `#` characters (`:75`), and builds the tree with a level stack (`build_tree_from_nodes` `:190`) — **no LLM at all** for the structure. The LLM is invoked only for *optional* node summaries and a doc description (`:267`, `:278`), which default to off in `md_to_tree` but are switched on when the tree is built through `PageIndexClient` (`client.py:99`). The README nonetheless discourages the Markdown mode for anything converted from a PDF (it loses hierarchy) and steers real documents to the PDF path plus the cloud OCR — so the LLM-constructed PDF path is the one that carries every marketing and benchmark claim.

**Finding (2.2).** For its primary PDF input, PageIndex's index is a *probabilistic LLM artifact* — inferred, not parsed, and self-graded (§2.3). That is not a defect for locating sections, but it is dispositive for the Civilization boundary: a structure an LLM inferred is exactly the kind of thing that "must not become truth or certification evidence by itself" [11]. The deterministic Markdown path narrows *how* a tree can be built, not the boundary conclusion — its output is still advisory document evidence, never truth.
...
But **building a PDF index requires an external LLM API** (the Markdown path needs one only for optional summaries, §2.2). Every PDF extraction step routes through `llm_completion()`/`llm_acompletion()` (`utils.py:33`, `:63`), thin wrappers over `litellm` that read `OPENAI_API_KEY` (with a `CHATGPT_API_KEY` alias, `:22`) and default to `gpt-4o-2024-11-20` (`config.yaml:1`; the Anthropic `claude-sonnet` alternative is present but commented out, `:2`). On a final failure the sync wrapper **returns an empty string rather than raising** (`:59`) — a fail-open degradation that silently yields a partial tree. Air-gapped use is *possible only* by pointing LiteLLM at a self-hosted OpenAI-compatible endpoint (LiteLLM's multi-provider routing supports this, and community forks demonstrate Ollama/vLLM backends), but that is **not the default and is not vendored** — out of the box, PageIndex phones OpenAI [1][9].

**Finding (2.5).** For the PDF path this is the exact inverse of MemPalace, which defaulted to *local ONNX embeddings* (TAI-RES-2026-004). PageIndex needs a reasoning-grade LLM merely to *construct* a PDF index — so a Civilization adapter would have to vendor the model path and prove network/secret denial under the Base Slice 0 adapter gate (§3.3), and the OSS `PyPDF2==3.0.1` pin is itself a mild staleness signal (PyPDF2 is the archived predecessor of `pypdf`).
...
| Deterministic / reproducible structure | PDF trees are LLM-extracted at `temperature=0`, non-deterministic across models/versions (Markdown trees are deterministic) | **Weak (PDF)** |
| Offline / air-gapped construction | A PDF index requires an external LLM API (default OpenAI); vectorless but not API-less | **Weak (needs vendoring)** |
