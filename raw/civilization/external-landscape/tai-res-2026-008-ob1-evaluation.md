...
- **The two Transpara hardening PRs** — PR #5 (`1d1d6e2`, verifiable + embedding-failure-safe `capture_thought`) and PR #6 (`1c4c906`, `deno check` resolution + empty-error guard) — the fork's most substantive server-safety commits, both re-read here code-anchored — PR #5 (2026-06-14) was already summarized in the prior page's Run-3 note, and PR #6 is the only commit after that page [1][6].
...
The fork's Transpara delta is entirely **deployment and server-safety** — local-Docker self-host + Supabase migration (PR #1), Kubernetes deployment + `content_fingerprint` dedup (PR #2), `getEmbedding` malformed-response guards (PR #3/#4), plus a self-host recipe and OpenRouter key-rotation docs — with **no feature or catalog divergence**. The two most substantive commits are PR #5/#6 — of which PR #6 is the only one after the 2026-06-14 page (PR #5 was already summarized in that page's Run-3 note) [1][6]:

- **PR #5 (`1d1d6e2`, merged 2026-06-14) — verifiable + embedding-failure-safe `capture_thought`.** The embedding is generated first (in a `Promise.all` with metadata), then `upsert_thought` commits the row, then the embedding is attached to the committed row by a separate best-effort `update` (`server/index.ts:493-505`); the old code returned an error for the already-saved thought when that attach failed (causing retry/duplication). The new capture contract (`capture-response.mjs:15-74`, priority-ordered) is **fail-closed**: (1) upsert error → error; (2) committed **but no id** → error *"Capture unconfirmed… Treat as NOT saved"* (`:46-54`); (3) committed, but the post-commit embedding *update* fails → **success**, `embedding: pending` (not an error — avoids duplicate-creating retries); (4) committed + embed ok → success `embedding: ready`. (An embedding-*generation* outage throws before the row is committed, so it is a hard error, not a `pending` save.) Success text now embeds the committed id + status so a client can confirm with `fetch(id)` (`:61`), and a non-string id collapses to `undefined` → the fail-closed branch (`server/index.ts:484-485`). The response shaping is a pure, infra-free module unit-tested by `test-capture-response.mjs`.
...
- **No hardcoded secrets.** The four sensitive vars (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `OPENROUTER_API_KEY`, `MCP_ACCESS_KEY`) are read via `Deno.env.get(...)` with **no fallback default** (`server/index.ts:13-16`) — no hardcoded secret, no silent default. Behavior on a missing var splits by var: `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` feed `createClient` at module load (`server/index.ts:19`), so a missing one fails at cold-start; `OPENROUTER_API_KEY` and `MCP_ACCESS_KEY` are read lazily and surface on first use (the TypeScript non-null assertion is erased at runtime and enforces nothing). The load-bearing posture is the absence of a fallback. [5]
...
[6] Transpara hardening PRs: `transpara-ai/OB1` PR #5 (`1d1d6e2`, verifiable + embedding-failure-safe `capture_thought`) and PR #6 (`1c4c906`, `deno check` resolution + empty-error guard). The most substantive of the fork's deployment/server-safety commits (PRs #1–#6); PR #6 is the only one after the 2026-06-14 page (PR #5 was already summarized in that page's Run-3 note).
