...
- **No hardcoded secrets.** The four sensitive vars (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `OPENROUTER_API_KEY`, `MCP_ACCESS_KEY`) are read via `Deno.env.get(...)` with **no fallback default** (`server/index.ts:13-16`) — no hardcoded secret, no silent default. (The TypeScript non-null assertion is erased at runtime, so a missing var surfaces on first use — e.g. `Authorization: Bearer undefined` on the first embedding call — not at startup; the load-bearing posture is the absence of a fallback, not a startup guard.) [5]
...
[6] Transpara hardening PRs: `transpara-ai/OB1` PR #5 (`1d1d6e2`, verifiable + embedding-failure-safe `capture_thought`) and PR #6 (`1c4c906`, `deno check` resolution + empty-error guard). The most substantive of the fork's deployment/server-safety commits (PRs #1–#6), and the only ones after the 2026-06-14 page.
