...
| 1.1.0 | 2026-07-07 | Planning traceability — on human instruction after merge (wiki PR #49, merge `a793c5f`), every §8 recommendation was filed as a source-of-intent issue in its owning repo; §8 items now carry their issue links so the research→planning provenance is manifest in the record itself. No findings, learnings, or determinations changed. | M. Saucier / Hive |
...
> **Planning traceability (v1.1.0, 2026-07-07):** every recommendation below was filed as a source-of-intent issue in its owning repo on 2026-07-07, on human instruction after this eval merged. Each issue is intake-only — it authorizes nothing, TLC applies, and readiness labeling remains human-set. The Neo envelope reference (§5.6, reference-only, not a step below) was additionally filed as [work#87](https://github.com/transpara-ai/work/issues/87).

1. **File the throttle issue** (hive): review-capacity WIP gate on work-start, per §5.1 — verify no existing equivalent, then intake via `cc:` labels as a normal TLC arc. → *filed as [hive#250](https://github.com/transpara-ai/hive/issues/250).*
2. **File the FO-lint issue** (docs/platform intake tooling): the five-predicate DoR + padding ban as Factory Order crafting checks, per §5.2. → *filed as [docs#260](https://github.com/transpara-ai/docs/issues/260) (standard first; platform tooling follows).*
3. **Schedule a density pass** over the CFAR/CFADA/TLC skill and runbook prose as docs hygiene, per §5.3 — semantics frozen, wording compressed. → *filed as [platform#48](https://github.com/transpara-ai/platform/issues/48).*
4. **Seed the repo-health harness proposal** (Level 0, read-only, proposals-only) from the Factory Standard's 12 buckets, per §5.4. → *filed as [platform#49](https://github.com/transpara-ai/platform/issues/49).*
5. **Add the single-mover and stale-claim invariants** to the cc:* label documentation, per §5.5. → *filed as [hive#251](https://github.com/transpara-ai/hive/issues/251).*
...
Repo defects surfaced by this eval's own gate arc: [wiki#50](https://github.com/transpara-ai/wiki/issues/50) (sanctioned register path for session-authored raw docs — **closed COMPLETED** 2026-07-06 via FO-WIKI-FRONTEND-UX R6; this document was the path's first registered consumer on 2026-07-07 at the v1.0.0 bytes; after this revision merges, the v1.1.0 bytes supersede that row via the authorized replace operation — a fresh register for the same path is rejected by design) and [wiki#52](https://github.com/transpara-ai/wiki/issues/52) (inherited CONFIDENTIAL frontmatter in ledgered inbox evals — open).
