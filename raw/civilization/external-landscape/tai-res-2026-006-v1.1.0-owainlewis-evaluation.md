...
version: 1.1.0
supersedes: raw/civilization/external-landscape/tai-res-2026-006-owainlewis-evaluation.md  # v1.0.0 — registered on the live ingest service 2026-07-07 (wiki#50 closure record; service-side ledger artifacts sync via the service's own arc); kept byte-identical per the tier's immutability rule
status: RELEASED
date: 2026-07-07
...
Repo defects surfaced by this eval's own gate arc: [wiki#50](https://github.com/transpara-ai/wiki/issues/50) (sanctioned register path for session-authored raw docs — **closed COMPLETED** 2026-07-06 via FO-WIKI-FRONTEND-UX R6; this document's v1.0.0 was the path's first registered consumer on 2026-07-07 (evidence: the wiki#50 closure record; the service-side shard/ledger artifacts reach the repo via the ingest service's own sync arc) and stays byte-identical at its path; this v1.1.0 revision is a NEW versioned file with `supersedes:` linkage, so the ledger row remains verifiable regardless of any later authorized replace/registration of the v1.1.0 ref (operator's option)) and [wiki#52](https://github.com/transpara-ai/wiki/issues/52) (inherited CONFIDENTIAL frontmatter in ledgered inbox evals — closed COMPLETED 2026-07-07 via the Option-1 supersession pass).
