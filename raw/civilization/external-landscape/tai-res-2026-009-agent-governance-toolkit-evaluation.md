...
|---|---|---|---|
| 1.0.1 | 2026-07-09 | CFAR (cross-family review, Codex) correction: narrowed the copyright-header provenance claim from "every source file" to "most source files" in §2.1 and the companion page — at pin `6dc05da` a few files (e.g. `agent-governance-golang/packages/agentmesh/identity.go`, `agent-os/src/agent_os/cli/cmd_policy_gen.py`) lack the header, so the universal overstated the code-anchored evidence. The verified count (128 under `agent-os/src`), the MIT license, and the separate "nothing Transpara-authored; fork inherited verbatim" facts are unchanged. | Claude (Opus 4.8) / Hive |
